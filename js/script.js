    ...
    async function exportCurrentTabSVG() {
        ...
        children.forEach(child => {
            try {
                ...
                // Prefer any inner icon span's styling if present
                ...
                // Also set style for variation settings if present on icon span
                if (iconSpan) {
                ...
                // Use icon span content if available (this is the ligature string for Material Symbols), otherwise element text
                const content = iconSpan ? (iconSpan.textContent || '').trim() : (child.textContent || '').trim();
                if (!content) return;
